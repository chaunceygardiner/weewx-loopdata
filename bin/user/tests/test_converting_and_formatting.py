...
class ConversionAndFormattingTests(unittest.TestCase):
    def test_METRIC_to_METRIC(self):
        converter, formatter = ConversionAndFormattingTests._get_converter_and_formatter('metric')
        pkt = {
            'dateTime'     : 12345689,
    ...
    def test_METRIC_to_US(self):
        converter, formatter = ConversionAndFormattingTests._get_converter_and_formatter('us')
        pkt = {
            'dateTime'     : 12345689,
            'usUnits'      : weewx.METRIC,
    ...
    def test_US_to_METRIC(self):
        ...
        user.loopdata.LoopProcessor.convert_units(converter, formatter, pkt, 'wind')
        self.assertEqual(pkt['FMT_HI_wind'], '6 kph')
    ...
    def test_METRICWX_to_METRIC(self):
        ...
        user.loopdata.LoopProcessor.convert_units(converter, formatter, pkt, 'wind')
        self.assertEqual(pkt['FMT_HI_wind'], '13 kph')
    ...
    @staticmethod
    def _get_converter_and_formatter(kind):
        config_dict = configobj.ConfigObj('bin/user/tests/weewx.conf.%s' % kind, encoding='utf-8')
