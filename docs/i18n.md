...
**German, French, Danish, Dutch and Spanish ship with the skin** — `skins/LoopData/lang/de.conf`
and `lang/fr.conf` (both native-speaker reviewed), `lang/da.conf` (contributed by
native speaker Gert Andersen), and `lang/nl.conf` and
`lang/es.conf` (Beta awaiting their reviews)
