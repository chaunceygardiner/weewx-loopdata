...
Swedish ship with the skin** — `skins/LoopData/lang/de.conf`
and `lang/fr.conf` (both native-speaker reviewed), `lang/da.conf` (contributed by
native speaker Gert Andersen), and `lang/nl.conf`, `lang/es.conf`,
`lang/it.conf`, `lang/no.conf` and `lang/sv.conf` (Beta awaiting their reviews)
are complete translations whose shared vocabulary is copied verbatim from
the lang files of
...
and a test keeps the three extensions in step.  Select one in `weewx.conf`:
