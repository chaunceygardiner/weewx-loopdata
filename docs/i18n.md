...
**German ships with the skin** — `skins/LoopData/lang/de.conf` is a
complete translation whose shared vocabulary is copied verbatim from the
native-speaker-reviewed German of
...
and a test keeps the three extensions in step.  Select it in `weewx.conf`:
