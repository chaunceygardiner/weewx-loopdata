...
* rolling 24 hour aggregate values (e.g., `24h.rain.sum`, `24h.windrun.sum`)
...
1. Download the lastest release, weewx-loopdata-2.12.zip, from the
   [GitHub Repository](https://github.com/chaunceygardiner/weewx-loopdata).

1. Run the following command.

   `sudo /home/weewx/bin/wee_extension --install weewx-loopdata-2.12.zip`
...
include the dollar sign).  Also, you can use the `2m`, `10m` or `24h` prefix to get aggregate values
for a rolling two minutes, ten minutes or twenty-four hours, respectively.
`2m`, `10m` and `24h` act just like `day`, `week`, `month`, `year` and `rainyear`.
