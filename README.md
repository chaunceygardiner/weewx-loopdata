...
containing observations from loop packets, along with day highs and
lows.  Typically, loop packets are generated frequently in WeeWX
(e.g, every 2s).  With the constantly updating loop-data.txt file, one
can write javascript to update a report in near real time.

LoopData adds all observations for which there are daily summaries and
the daily high and low for those observations.  Since loop data examines loop
packets to decide what data to write, it is easily extensible.  For example,
if the [weewx-purple](https://github.com/chaunceygardiner/weewx-purple)
...
Another convenience LoopData provides is that a WeeWx report is specified
in the configuration (e.g., `SeasonsReport`).  With this information,
LoopData automatically converts all observations to the units called for
in the report and also formats all readings according to the report
specified.  Thus, it is simple to replace the reports observations in
javascript as they will already be in the correct units and in the correct
format.
